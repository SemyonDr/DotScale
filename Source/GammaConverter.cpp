...
/// </summary>
ImageBuffer_uint16* GammaConverter::RemoveGammaCorretion(ImageBuffer_Byte* image) {
	//Aliases
	...
	ImageBuffer_uint16* linear_image = new ImageBuffer_uint16(image_height, image_width, image->GetLayout());
	uint16_t** linear_data = linear_image->GetData(); //Alias
...
/// </summary>
ImageBuffer_Byte* GammaConverter::ApplyGammaCorrection(ImageBuffer_uint16* linear_image, BitDepth bitDepth) {
	//Aliases
	int image_height = linear_image->GetHeight();
	int image_width = linear_image->GetWidth();
	uint16_t** linear_data = linear_image->GetData();

	//Allocating resulting image
	ImageBuffer_Byte* image = new ImageBuffer_Byte(image_height, image_width, linear_image->GetLayout(), bitDepth);
	uint8_t** data = image->GetData(); //Alias
